-- Migración 052: Agregar parámetro loan_type a función create_p2p_loan
-- Permite especificar explícitamente si es préstamo de dinero o de objeto
...
CREATE OR REPLACE FUNCTION create_p2p_loan(
  ...
  p_loan_type VARCHAR DEFAULT 'unknown'  -- NUEVO: 'money', 'object', 'unknown'
) RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  -- Validar y convertir loan_type
  BEGIN
    v_loan_type := p_loan_type::loan_type;
  EXCEPTION WHEN invalid_text_representation THEN
    v_loan_type := 'unknown'::loan_type;
  END;
  ...
  RAISE NOTICE '[create_p2p_loan] Lender: %, Borrower: %, I am lender: %, loan_type: %',
    v_lender_tenant_id, v_borrower_tenant_id, p_i_am_lender, v_loan_type;
  ...
  -- 5. Crear agreement con loan_type y status 'pending_confirmation'
  INSERT INTO agreements (
    ...
    type,
    loan_type,             -- NUEVO: tipo de préstamo (money/object/unknown)
    title,
    description,
    amount,
    currency,
  ...
  RAISE NOTICE '[create_p2p_loan] Agreement creado: % con loan_type: %', v_agreement_id, v_loan_type;
...
DO $$
BEGIN
  RAISE NOTICE 'Migración 052 completada:';
  RAISE NOTICE '- Función create_p2p_loan actualizada con parámetro loan_type';
  RAISE NOTICE '- Valores válidos: money, object, unknown (default)';
END $$;
