-- Migración 045: Agregar created_by a create_p2p_loan
-- Corrige error: "null value in column created_by violates not-null constraint"
...
DECLARE
  ...
  v_created_by_user_id UUID;  -- NUEVO: para almacenar el user_id
BEGIN
  ...
  -- 2b. NUEVO: Obtener el owner user del tenant para created_by
  SELECT id
  ...
  -- 5. Crear agreement con created_by
  ...
  RAISE NOTICE '[create_p2p_loan] Agreement creado: % con created_by: %', v_agreement_id, v_created_by_user_id;
...
DO $$
BEGIN
  RAISE NOTICE 'Migración 045 completada:';
  RAISE NOTICE '- Función create_p2p_loan actualizada';
  RAISE NOTICE '- Ahora incluye created_by en INSERT de agreements';
  RAISE NOTICE '- Obtiene owner user automáticamente desde tenant';
END $$;
