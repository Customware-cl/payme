-- Migración 036: Strip semicolon from query before execution
--
-- PROBLEMA: El SQL generator produce queries que terminan con ';'
-- lo cual es válido en SQL, pero cuando envolvemos el query en una
-- subquery para aplicar LIMIT, el ';' causa un error de sintaxis:
--
--   SELECT ... FROM (SELECT ... ; LIMIT 100) t
--                                ↑ syntax error
--
-- SOLUCIÓN: Eliminar ';' del final del query antes de ejecutarlo
-- usando rtrim(sql_query, ';')
...
BEGIN
  ...
  -- =====================================================
  BEGIN
    -- Ejecutar query limpio (sin semicolon) y convertir resultado a JSON
    ...
    -- VALUES (cleaned_query, row_count, NOW());
...
Timeout: 10 segundos.
Máximo: 1000 filas.
v2.0.11: Fix de regex bug - usa LIKE para validación de SELECT.
v2.0.12: Strip semicolon del final antes de ejecutar para evitar syntax errors en subquery.
