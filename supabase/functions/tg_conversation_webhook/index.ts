// Edge Function: Telegram Conversation Webhook
// Webhook completo con ConversationManager adaptado para Telegram

import { serve } from "https://deno.land/std@0.177.0/http/server.ts";
import { createClient } from "https://esm.sh/@supabase/supabase-js@2";
import { TelegramConversationManager } from "../_shared/telegram-conversation-manager.ts";
...
async function processInboundMessage(
  ...
  try {
    ...
    if (!contactId) {
      ...
      const { data: newContact, error: contactError } = await supabase
        ...
        .insert({
          ...
          telegram_last_name: update.message?.from?.last_name || update.callback_query?.from?.last_name || null,
          name: userName,
    ...
    // 4. Procesar mensaje usando TelegramConversationManager
    ...
    if (update.message?.text) {
      ...
      if (lowerText === 'hola' || lowerText === 'hi' || lowerText === '/start') {
        ...
        const conversationManager = new TelegramConversationManager(supabase.supabaseUrl, supabase.supabaseKey);
      ...
      // Si no hay respuesta específica, procesar con TelegramConversationManager
      if (!responseMessage) {
        try {
          const conversationManager = new TelegramConversationManager(supabase.supabaseUrl, supabase.supabaseKey);
      ...
      switch (callbackData) {
        ...
        default:
          // Si es un callback desconocido, pasarlo al TelegramConversationManager como texto
          const conversationManager = new TelegramConversationManager(supabase.supabaseUrl, supabase.supabaseKey);
...
serve(async (req: Request) => {
  try {
    ...
    if (req.method === 'GET') {
      return new Response('Telegram conversation webhook is running', {
        status: 200,
        headers: {
